   ...
   stages {
      ...
      stage('Deploy') {
        steps {
         ...
         sh 'echo {\"ID\": \"check-${VERSION}\",\"Name\": \"check-${VERSION}\",\"HTTP\": \"http://172.31.29.255:${PORT}\",\"Method\": \"GET\",\"ServiceID\": \"back-${VERSION}\",\"Interval\": \"10s\",\"TLSSkipVerify\": true} > payload.json' 
        }
   }
